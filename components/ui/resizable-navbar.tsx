    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-1 py-2 text-neutral-600 dark:text-neutral-300"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
...
export const NavbarLogo = ({ logo, name }: { logo?: string; name?: string }) => {
  return (
    <a href="#" className="relative z-20 mr-4 flex items-center space-x-2 px-2 py-1 text-sm font-normal text-black">
      {logo && <img src={logo || "/placeholder.svg"} alt={name || "logo"} width={30} height={30} />}
      {name && <span className="font-medium text-gray-700 dark:text-white">{name}</span>}
    </a>
  )
}
